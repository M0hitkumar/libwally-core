...
public class test_assets {
    ...
    private void test_confidential_values() {
        ...
        for (int i = 0; i < long_values.length; ++i) {
          if (!hex_values[i].equals(h(Wally.tx_confidential_value_from_bronee(long_values[i]))))
              throw new RuntimeException("Unexpected confidential value");

          if (long_values[i] != Wally.tx_confidential_value_to_bronee(h(hex_values[i])))
              throw new RuntimeException("Unexpected long bronee value");
        }
    }
