...
NETWORK_BROCOIN_MAINNET = 0x01
NETWORK_BROCOIN_TESTNET = 0x02
...
class AddressTests(unittest.TestCase):
    ...
    def test_address_vectors(self):
        self.do_test_vector(vec, 'm/0H/1', NETWORK_BROCOIN_MAINNET)
        self.do_test_vector(vec, 'm/1H/1', NETWORK_BROCOIN_TESTNET) # Testnet

    def do_test_vector(self, vec, path, network):
        key = self.get_test_key(vec, path)

        # Address type flag is mandatory
        version = ADDRESS_VERSION_P2PKH_MAINNET if network == NETWORK_BROCOIN_MAINNET else ADDRESS_VERSION_P2PKH_TESTNET
        ...
        version = ADDRESS_VERSION_P2SH_MAINNET if network == NETWORK_BROCOIN_MAINNET else ADDRESS_VERSION_P2SH_TESTNET
        ...
        bech32_prefix = 'bc' if network == NETWORK_BROCOIN_MAINNET else 'tb'
