...
class PeginTests(unittest.TestCase):
    ...
    def test_pegin_tx(self):
        ...
        # proof from the bron transaction spending to mainchain address (TODO: generate these in wally)
        ...
        self.assertEqual(wally_tx_confidential_value_from_bronee(10**8, value, value_len), WALLY_OK)

        # the BRON transaction spending to mainchain address
        bron_tx_hex = ''
        tx_p = pointer(wally_tx())
        self.assertEqual(wally_tx_from_hex(bron_tx_hex, 0, tx_p), WALLY_OK)

        # strip the witness data from the BRON transaction
        ret, length = wally_tx_get_length(tx_p[0], 0)
        self.assertEqual(ret, WALLY_OK)
        tx_bron_no_witness, tx_bron_no_witness_len = make_cbuffer('00'*length)
        self.assertEqual((WALLY_OK, length), wally_tx_to_bytes(tx_p[0], 0, tx_bron_no_witness, tx_bron_no_witness_len))
        ...
        unconfidential_bronee, unconfidential_bronee_len = make_cbuffer('00'*9)
        self.assertEqual(wally_tx_confidential_value_from_bronee(99996550, unconfidential_bronee, unconfidential_bronee_len), WALLY_OK)
        ...
        self.assertEqual(wally_tx_add_elements_raw_output(tx, script, script_len, b'\x01'+asset[::-1], asset_len+1,
                                                          unconfidential_bronee, unconfidential_bronee_len, None, 0, None, 0, None, 0, 0), WALLY_OK)

        self.assertEqual(wally_tx_confidential_value_from_bronee(3450, unconfidential_bronee, unconfidential_bronee_len), WALLY_OK)
        ...
        # sign transaction
        unconfidential_bronee, unconfidential_bronee_len = make_cbuffer('00'*9)
        self.assertEqual(wally_tx_confidential_value_from_bronee(10**8, unconfidential_bronee, unconfidential_bronee_len), WALLY_OK)
        ...
        self.assertEqual(wally_tx_get_elements_signature_hash(tx, 0, redeem_script, redeem_script_len,
                                                              unconfidential_bronee, unconfidential_bronee_len, 1, 1, signature_hash, signature_hash_len), WALLY_OK)
