...
static v8::Local<v8::Object> TransactionToObject(const struct wally_tx *tx, int ret)
{
    ...
    for(i = 0; i < tx->num_outputs; i++) {
        ...
        if(tx->outputs[i].bronee > 0) {
            Nan::Set(output, Nan::New("bronee").ToLocalChecked(), Nan::New<v8::String>(std::to_string(tx->outputs[i].bronee)).ToLocalChecked());
        } else {
            Nan::Set(output, Nan::New("bronee").ToLocalChecked(), Nan::New<v8::String>(std::to_string(0)).ToLocalChecked());
        }
