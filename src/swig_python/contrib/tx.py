...
class TxTests(unittest.TestCase):
    ...
    def test_tx_output(self):
        ...
        bronee, script = 10000, b'0000'

        # Create a valid output
        tx_output = tx_output_init(bronee, script)
        self.assertEqual(tx_output_get_bronee(tx_output), bronee)
    ...
    def test_tx_set_output(self):
        bronee, script = WALLY_BRONEE_MAX, b'0000'

        # Create tx with single output value = MAX
        tx = tx_init(2, 0, 10, 2)
        tx_add_output(tx, tx_output_init(bronee, script))
        self.assertEqual(tx_get_output_bronee(tx, 0), WALLY_BRONEE_MAX)
        self.assertEqual(tx_get_total_output_bronee(tx), WALLY_BRONEE_MAX)

        # Change value of output from MAX -> 1
        tx_set_output_bronee(tx, 0, 1)
        self.assertEqual(tx_get_output_bronee(tx, 0), 1)
        self.assertEqual(tx_get_total_output_bronee(tx), 1)
    ...
    def test_tx(self):
        ...
        self.assertEqual(tx_get_total_output_bronee(tx), 0)
        ...
        self.assertEqual(tx_get_total_output_bronee(tx), 10000)
        tx_add_raw_output(tx, 20000, script, 0)
        self.assertEqual(tx_get_total_output_bronee(tx), 30000)
        size = tx_get_length(tx, 0)
        vsize = tx_vsize_from_weight(tx_get_weight(tx))
        tx_hex = tx_to_hex(tx, FLAG_USE_WITNESS)

        with self.assertRaises(ValueError):
            tx_add_raw_output(tx, WALLY_BRONEE_MAX + 1, script, 0)
        with self.assertRaises(ValueError):
            total_to_overflow = WALLY_BRONEE_MAX - tx_get_total_output_bronee(tx) + 1
