...
#define MAX_INVALID_BRONEE ((uint64_t) -1)
...
struct tx_serialize_opts
{
    ...
    uint64_t bronee;                /* Amount of the input we are signing */
...
#define WALLY_BRONEE_MAX ((uint64_t)WALLY_BRON_MAX * WALLY_BRONEE_PER_BRON)
...
static bool is_valid_tx_output(const struct wally_tx_output *output)
{
    return output &&
           ...
           output->bronee <= WALLY_BRONEE_MAX;
}
    ...
    bool is_elements)
{
    ...
    if (BYTES_INVALID(script, script_len) || !output ||
        (bronee > WALLY_BRONEE_MAX && !is_elements))
        return WALLY_EINVAL;
...
int wally_tx_elements_output_init(
    ...
    struct wally_tx_output *output)
{
    return tx_elements_output_init(MAX_INVALID_BRONEE, script, script_len,
...
int wally_tx_output_init(uint64_t bronee,
                         const unsigned char *script, size_t script_len,
                         struct wally_tx_output *output)
{
    return tx_elements_output_init(bronee, script, script_len,
                                   NULL, 0, NULL, 0, NULL, 0,
                                   NULL, 0, NULL, 0, output, false);
}

int wally_tx_output_init_alloc(uint64_t bronee,
                               const unsigned char *script, size_t script_len,
                               struct wally_tx_output **output)
{
    struct wally_tx_output *result;
    ...
    ret = wally_tx_output_init(bronee, script, script_len, result);
                           ...
                           const struct wally_tx_output *output)
{
    ...
    if (!is_elements) {
        if (!is_valid_tx_output(output) ||
            wally_tx_get_total_output_bronee(tx, &total) != WALLY_OK ||
            total + output->bronee < total || total + output->bronee > WALLY_BRONEE_MAX)
            return WALLY_EINVAL;
...
static int tx_add_elements_raw_output_at(
    struct wally_tx *tx, uint32_t index, uint64_t bronee,
    const unsigned char *script, size_t script_len,
    ...
    uint32_t flags, bool is_elements)
{
    ...
    struct wally_tx_output output = {
        bronee, (unsigned char *)script, script_len,
        is_elements ? WALLY_TX_IS_ELEMENTS : 0,
#ifdef BUILD_ELEMENTS
...
int wally_tx_add_raw_output(struct wally_tx *tx, uint64_t bronee,
                            const unsigned char *script, size_t script_len,
                            uint32_t flags)
{
    if (!tx)
        return WALLY_EINVAL;
    return tx_add_elements_raw_output_at(tx, tx->num_outputs, bronee,
                                         script, script_len,
                                         NULL, 0, NULL, 0, NULL, 0,
...
int wally_tx_add_raw_output_at(struct wally_tx *tx, uint32_t index,
                               ...
                               uint32_t flags)
{
    return tx_add_elements_raw_output_at(tx, index, bronee,
                                         script, script_len,
                                         NULL, 0, NULL, 0, NULL, 0,
                                         NULL, 0, NULL, 0, flags, false);
}
                                     ...
                                     size_t *written)
{
    ...
    if (!is_elements)
        p += uint64_to_le_bytes(opts->bronee, p);
#ifdef BUILD_ELEMENTS
    else
    ...
    if (sh_none || (sh_single && opts->index >= tx->num_outputs))
        ...
        for (i = 0; i < tx->num_outputs; ++i) {
            if (sh_single && i != opts->index)
                continue;
            if (!is_elements)
                output_p += uint64_to_le_bytes(tx->outputs[i].bronee, output_p);
#ifdef BUILD_ELEMENTS
            else {
        ...
        for (i = 0; i < num_outputs; ++i) {
            ...
            if (sh_single && i != opts->index) {
                ...
                if (output->features & WALLY_TX_IS_ELEMENTS) {
                    ...
                    p += uint64_to_le_bytes(output->bronee, p);
                }
                         ...
                         uint32_t flags, struct wally_tx **output)
{
    ...
    for (i = 0; i < num_outputs; ++i) {
        ...
        uint64_t bronee = -1, script_len, asset_len = 0, value_len = 0, nonce_len = 0;
        if (is_elements) {
            asset = p;
            ...
            p += uint64_from_le_bytes(p, &bronee);
        p += varint_from_bytes(p, &script_len);
        script = p;
        p += script_len;
        ret = tx_elements_output_init(bronee, script_len ? script : NULL, script_len,
...
static int tx_get_signature_hash(const struct wally_tx *tx,
                                 ...
                                 uint32_t extra_offset, uint64_t bronee,
                                 const unsigned char *value,
                                 size_t value_len,
                                 ...
                                 unsigned char *bytes_out, size_t len)
{
    ...
    int ret;
    const struct tx_serialize_opts opts = {
        sighash, tx_sighash, index, script, script_len, bronee,
    ...
    if (!is_valid_tx(tx) || BYTES_INVALID(script, script_len) ||
        ...
        bronee > WALLY_BRONEE_MAX || (sighash & 0xffffff00) ||
                                ...
                                unsigned char *bytes_out, size_t len)
{
    return tx_get_signature_hash(tx, index, script, script_len,
                                 extra, extra_len, extra_offset, bronee,
                                 NULL, 0, sighash, tx_sighash, flags, bytes_out, len);
}

int wally_tx_get_bron_signature_hash(const struct wally_tx *tx, size_t index,
                                    const unsigned char *script, size_t script_len,
                                    uint64_t bronee, uint32_t sighash, uint32_t flags,
                                    unsigned char *bytes_out, size_t len)
{
    return wally_tx_get_signature_hash(tx, index, script, script_len,
                                       NULL, 0, 0, bronee, sighash, sighash,
                                       flags, bytes_out, len);
}
...
int wally_tx_confidential_value_from_bronee(uint64_t bronee,
                                             ...
                                             size_t len)
{
    ...
    *bytes_out = 0x1;
    uint64_to_be_bytes(bronee, &bytes_out[1]);

    return WALLY_OK;
}

int wally_tx_confidential_value_to_bronee(const unsigned char *value,
                                           size_t value_len,
                                           uint64_t *value_out)
{
...
int wally_tx_get_total_output_bronee(const struct wally_tx *tx, uint64_t *value_out)
{
    size_t i;
    if (value_out)
        *value_out = 0;
    ...
    for (i = 0; i < tx->num_outputs; ++i) {
        uint64_t v = *value_out + tx->outputs[i].bronee;

        if (tx->outputs[i].bronee > WALLY_BRONEE_MAX ||
            v < *value_out || v > WALLY_BRONEE_MAX) {
            /* Overflow or too many bronee in outputs */
            *value_out = 0;
            return WALLY_EINVAL;
        }
        *value_out = v;
...
GET_TX_I(tx_output, bronee, uint64_t)
...
int wally_tx_output_set_bronee(struct wally_tx_output *output, uint64_t bronee)
{
    if (!is_valid_tx_output(output) || bronee > WALLY_BRONEE_MAX)
        return WALLY_EINVAL;
    output->bronee = bronee;
    return WALLY_OK;
}
...
int wally_tx_get_output_bronee(const struct wally_tx *tx, size_t index, uint64_t *value_out)
{
    return wally_tx_output_get_bronee(tx_get_output(tx, index), value_out);
}
...
int wally_tx_set_output_bronee(const struct wally_tx *tx, size_t index, uint64_t bronee)
{
    uint64_t current, total;

    if (wally_tx_get_output_bronee(tx, index, &current) != WALLY_OK ||
        wally_tx_get_total_output_bronee(tx, &total) != WALLY_OK)
        return WALLY_EINVAL;
    total -= current;
    if (total + bronee < total || total + bronee > WALLY_BRONEE_MAX)
        return WALLY_EINVAL;
    return wally_tx_output_set_bronee(tx_get_output(tx, index), bronee);
}
