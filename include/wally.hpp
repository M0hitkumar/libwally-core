...
template <class BYTES, class BYTES_OUT>
inline int format_brocoin_message(const BYTES& bytes, uint32_t flags, BYTES_OUT& bytes_out, size_t* written = 0) {
    size_t n;
    int ret = ::wally_format_brocoin_message(bytes.data(), bytes.size(), flags, bytes_out.data(), bytes_out.size(), written ? written : &n);
    return written || ret != WALLY_OK ? ret : n == static_cast<size_t>(bytes_out.size()) ? WALLY_OK : WALLY_EINVAL;
}
...
inline int tx_add_raw_output(const TX& tx, uint64_t bronee, const SCRIPT& script, uint32_t flags) {
    int ret = ::wally_tx_add_raw_output(detail::get_p(tx), bronee, script.data(), script.size(), flags);
    return ret;
}

template <class TX, class SCRIPT>
inline int tx_add_raw_output_at(const TX& tx, uint32_t index, uint64_t bronee, const SCRIPT& script, uint32_t flags) {
    int ret = ::wally_tx_add_raw_output_at(detail::get_p(tx), index, bronee, script.data(), script.size(), flags);
    return ret;
}
...
inline int tx_get_bron_signature_hash(const TX& tx, size_t index, const SCRIPT& script, uint64_t bronee, uint32_t sighash, uint32_t flags, BYTES_OUT& bytes_out) {
    int ret = ::wally_tx_get_bron_signature_hash(detail::get_p(tx), index, script.data(), script.size(), bronee, sighash, flags, bytes_out.data(), bytes_out.size());
    return ret;
}
...
inline int tx_get_signature_hash(const TX& tx, size_t index, const SCRIPT& script, const EXTRA& extra, uint32_t extra_offset, uint64_t bronee, uint32_t sighash, uint32_t tx_sighash, uint32_t flags, BYTES_OUT& bytes_out) {
    int ret = ::wally_tx_get_signature_hash(detail::get_p(tx), index, script.data(), script.size(), extra.data(), extra.size(), extra_offset, bronee, sighash, tx_sighash, flags, bytes_out.data(), bytes_out.size());
    return ret;
}

template <class TX>
inline int tx_get_total_output_bronee(const TX& tx, uint64_t* value_out) {
    int ret = ::wally_tx_get_total_output_bronee(detail::get_p(tx), value_out);
    return ret;
}
...
inline int tx_output_init(uint64_t bronee, const SCRIPT& script, struct wally_tx_output* output) {
    int ret = ::wally_tx_output_init(bronee, script.data(), script.size(), output);
    return ret;
}

template <class SCRIPT>
inline int tx_output_init_alloc(uint64_t bronee, const SCRIPT& script, struct wally_tx_output** output) {
    int ret = ::wally_tx_output_init_alloc(bronee, script.data(), script.size(), output);
    return ret;
}
...
template <class BYTES_OUT>
inline int tx_confidential_value_from_bronee(uint64_t bronee, BYTES_OUT& bytes_out) {
    int ret = ::wally_tx_confidential_value_from_bronee(bronee, bytes_out.data(), bytes_out.size());
    return ret;
}

template <class VALUE>
inline int tx_confidential_value_to_bronee(const VALUE& value, uint64_t* value_out) {
    int ret = ::wally_tx_confidential_value_to_bronee(value.data(), value.size(), value_out);
    return ret;
}
