...
#define WALLY_BRONEE_PER_BRON 100000000
#define WALLY_BRON_MAX 21000000
 ...
 *
 * :param bronee The amount of the output in bronee.
 * :param script: The scriptPubkey for the output.
 * :param script_len: Size of ``script`` in bytes.
 * :param output: Transaction output to initialize.
 */
WALLY_CORE_API int wally_tx_output_init(uint64_t bronee,
                                        const unsigned char *script,
                                        size_t script_len,
 ...
 */
WALLY_CORE_API int wally_tx_get_total_output_bronee(
    const struct wally_tx *tx,
    uint64_t *value_out);

/**
 * Create a BRON transaction for signing and return its hash.
 *
 ...
 * :param script_len: Size of ``script`` in bytes.
 * :param bronee: The amount spent by the input being signed for. Only used if
 ...
 *|     This must be given in host CPU endianess; For normal Brocoin signing
...
/**
 * Convert bronee to an explicit confidential value representation.
 *
 * :param bronee: The value in bronee to convert.
 ...
 */
WALLY_CORE_API int wally_tx_confidential_value_from_bronee(
    uint64_t bronee,
    unsigned char *bytes_out,
    size_t len);

/**
 * Convert an explicit confidential value representation to bronee.
 *
 ...
 * :param value_out: The converted value in bronee.
 */
WALLY_CORE_API int wally_tx_confidential_value_to_bronee(
    const unsigned char *value,
    size_t value_len,
    uint64_t *value_out);
