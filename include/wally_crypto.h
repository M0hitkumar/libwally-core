...
#define BROCOIN_MESSAGE_MAX_LEN (64 * 1024 - 64)

/** Indicates that SHA256D(message) should be returned */
#define BROCOIN_MESSAGE_FLAG_HASH 1

/**
 * Format a message for use as a brocoin signed message.
 *
 * :param bytes: The message string to sign.
 * :param bytes_len: The length of ``bytes`` in bytes. Must be less than
 *|    or equal to BROCOIN_MESSAGE_MAX_LEN.
 * :param flags: BROCOIN_MESSAGE_FLAG_ flags indicating the desired output.
 *|    if BROCOIN_MESSAGE_FLAG_HASH is passed, the double SHA256 hash
 ...
 * :param written: Destination for the number of bytes written to ``bytes_out``.
 */
WALLY_CORE_API int wally_format_brocoin_message(
    const unsigned char *bytes,
    size_t bytes_len,
    uint32_t flags,
    unsigned char *bytes_out,
